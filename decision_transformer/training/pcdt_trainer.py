...
class Trainer:
    ...
    def train_step(self, log_writer=None, loss_metric={}):
        ...
        # regloss
        _, pi_preds, _ = self.actor.forward(
            states, actions, rewards, action_target, rtg[:, :-1], timesteps, attention_mask=attention_mask,
        )
        _, hehav, _ = self.bcmodel.forward(
            states, actions, rewards, action_target, rtg[:, :-1], timesteps, attention_mask=attention_mask,
        )
        mask_pi = pi_preds[:, :-1][attention_mask[:, :-1] > 0]
        mask_behav = hehav[:, :-1][attention_mask[:, :-1] > 0]
        mask_actions = actions[:, :-1][attention_mask[:, :-1] > 0]

        behav_prob = torch.exp(-torch.mean((mask_behav - mask_actions) ** 2, dim=1) / self.inv_gauss_coef)
        pi_prob = torch.exp(-torch.mean((mask_pi - mask_actions) ** 2, dim=1) / self.inv_gauss_coef)

        isratio = torch.clamp(pi_prob / behav_prob, min=0.1)
        ...
        curr_Q = torch.cat([current_q1[:, :-1][attention_mask[:, :-1] > 0], current_q2[:, :-1][attention_mask[:, :-1] > 0]], dim=1)
        pi_q1, pi_q2 = self.critic(states, pi_preds)
        pi_Q = torch.cat([pi_q1[:, :-1][attention_mask[:, :-1] > 0], pi_q2[:, :-1][attention_mask[:, :-1] > 0]], dim=1)
        qmin = (torch.ones_like(curr_Q) * self.Q_min).detach()
        reg_loss = torch.clamp((pi_Q-qmin)**2 - weight * (curr_Q-qmin)**2, min=-1e4)
        ...
        #     states, actions, rewards, action_target, rtg[:, :-1], timesteps, attention_mask=attention_mask,
        # )
        state_preds, action_preds, _ = self.actor.forward(
            states, actions, rewards, action_target, rtg[:, :-1], timesteps, attention_mask=attention_mask,
        )
